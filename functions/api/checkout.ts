...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    try {
          ...
          console.log('checkout: start', { hasToken: !!env.SQUARE_ACCESS_TOKEN, hasLocation: !!env.SQUARE_LOCATION_ID })
      ...
      console.log('checkout: calling Square', { productId, locationId: locationId.substring(0, 8) + '...', redirectUrl })
      ...
      const data = await r.json().catch(() => null)
          if (!r.ok) {
                  console.error('Square API error', r.status, JSON.stringify(data))
      ...
      const checkoutUrl = data?.payment_link?.url
          ...
          console.error('checkout: exception', e instanceof Error ? e.message : String(e))
